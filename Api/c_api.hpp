...
//NEEDS C++ 17
//THE EXAMPLE CONTAINS STRING XOR'ING
...
namespace c_auth {
	namespace c_encryption {
		...
		inline std::string encrypt(std::string message, std::string enc_key, std::string iv = "default_iv") {
			...
			if (iv == "default_iv")
				return encrypt_string(message, enc_key, "1514834626578394");
			else
		...
		inline std::string decrypt(std::string message, std::string enc_key, std::string iv = "default_iv") {
			...
			if (iv == "default_iv")
				return decrypt_string(message, enc_key, "1514834626578394");
			else
	...
	namespace c_api {
		...
		inline std::string api_link = "https://cauth.me/api/";
		inline std::string user_agent = "Mozilla cAuth";
		...
		inline char* pub_key() {
			return (char*)"sha256//Mk6vhbkCoRzUhXoUryC8tjIxmehtu4uLVhwqGQM9Cmc=";
		}
		...
		inline void c_init(std::string c_version, std::string c_program_key, std::string c_encryption_key) {
			...
			try {
				...
				CURLcode code;
				if (c_url) {
					curl_easy_setopt(c_url, CURLOPT_URL, std::string(api_link + "handler.php?type=init").c_str());
					curl_easy_setopt(c_url, CURLOPT_USERAGENT, user_agent.c_str());

					curl_easy_setopt(c_url, CURLOPT_NOPROXY, "cauth.me");
					...
					std::string values =
						...
						"&api_version=" + c_encryption::encrypt("3.3b", enc_key) +
						"&program_key=" + c_encryption::hex_encode(program_key);
					...
					if (result != "program_doesnt_exist") {
						...
						if (result == "killswitch_is_enabled") {
							MessageBoxA(0, "The killswitch of the program is enabled, contact the developer", "cAuth", MB_ICONERROR);
							exit(0);
						}
						else if (result.find("wrong_version") != std::string::npos) {
							MessageBoxA(0, "Wrong program version", "cAuth", MB_ICONERROR);
						...
						MessageBoxA(0, "The program doesnt exist", "cAuth", MB_ICONERROR);
						exit(0);
					}
				}
			}
		...
		inline bool c_login(std::string c_username, std::string c_password, std::string c_hwid = "default") {
			...
			try {
				...
				CURLcode code;
				if (c_url) {
					curl_easy_setopt(c_url, CURLOPT_URL, std::string(api_link + "handler.php?type=login").c_str());
					curl_easy_setopt(c_url, CURLOPT_USERAGENT, user_agent.c_str());
					...
					std::string values =
						...
						"&sessid=" + c_encryption::hex_encode(session_id);
					...
					if (result == "killswitch_is_enabled") {
						...
						MessageBoxA(0, "Invalid username", "cAuth", MB_ICONERROR);
						return false;
					}
					else if (result == "invalid_password") {
						MessageBoxA(0, "Invalid password", "cAuth", MB_ICONERROR);
						return false;
					}
					else if (result == "user_is_banned") {
						MessageBoxA(0, "The user is banned", "cAuth", MB_ICONERROR);
						return false;
					}
					else if (result == "no_sub") {
						MessageBoxA(0, "Your subscription is over", "cAuth", MB_ICONERROR);
						return false;
					}
					else if (result == "invalid_hwid") {
						MessageBoxA(0, "Invalid HWID", "cAuth", MB_ICONERROR);
						return false;
					}
						...
						MessageBoxA(0, "Invalid encryption key/iv or session expired", "cAuth", MB_ICONERROR);
						return false;
					}
				}
			}
		...
		inline bool c_register(std::string c_username, std::string c_email, std::string c_password, std::string c_token, std::string c_hwid = "default") {
			...
			try {
				...
				CURLcode code;
				if (c_url) {
					curl_easy_setopt(c_url, CURLOPT_URL, std::string(api_link + "handler.php?type=register").c_str());
					curl_easy_setopt(c_url, CURLOPT_USERAGENT, user_agent.c_str());
					...
					if (result == "killswitch_is_enabled") {
						...
						MessageBoxA(0, "User already exists", "cAuth", MB_ICONERROR);
						return false;
					}
					else if (result == "email_already_exists") {
						MessageBoxA(0, "Email already exists", "cAuth", MB_ICONERROR);
						return false;
					}
					else if (result == "invalid_email_format") {
						MessageBoxA(0, "Invalid email format", "cAuth", MB_ICONERROR);
						return false;
					}
					else if (result == "invalid_token") {
						MessageBoxA(0, "Invalid token", "cAuth", MB_ICONERROR);
						return false;
					}
					else if (result == "maximum_users_reached") {
						MessageBoxA(0, "Maximum users of the program was reached, please contact the program owner", "cAuth", MB_ICONERROR);
						return false;
					}
					else if (result == "used_token") {
						MessageBoxA(0, "Already used token", "cAuth", MB_ICONERROR);
						return false;
					}
					else if (result == "success") {
						MessageBoxA(0, "Success!!", "cAuth", MB_ICONINFORMATION);
						return true;
					}
					else {
						MessageBoxA(0, "Invalid encryption key/iv or session expired", "cAuth", MB_ICONERROR);
		...
		inline bool c_activate(std::string c_username, std::string c_password, std::string c_token) {
			...
			try {
				...
				CURLcode code;
				if (c_url) {
					curl_easy_setopt(c_url, CURLOPT_URL, std::string(api_link + "handler.php?type=activate").c_str());
					curl_easy_setopt(c_url, CURLOPT_USERAGENT, user_agent.c_str());
		...
		inline std::string c_var(std::string c_var_name, std::string c_hwid = "default") {
			...
			try {
				...
				CURLcode code;
				if (c_url) {
					curl_easy_setopt(c_url, CURLOPT_URL, std::string(api_link + "handler.php?type=var").c_str());
					curl_easy_setopt(c_url, CURLOPT_USERAGENT, user_agent.c_str());
		...
		inline void c_log(std::string c_message) {
			...
			try {
				...
				CURLcode code;
				if (c_url) {
					curl_easy_setopt(c_url, CURLOPT_URL, std::string(api_link + "handler.php?type=log").c_str());
					curl_easy_setopt(c_url, CURLOPT_USERAGENT, user_agent.c_str());
